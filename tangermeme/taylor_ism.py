...
def _predict_grad(
    ...
    """An internal function for calculating the Taylor approximated ISM
    batched gradients in a memory-efficient manner.

    This function, which is meant to be used for Taylor approximated ISM, will
    take a PyTorch model and calculate gradients from it using forward function
    of the model and Pytorch autograd. The optional additional arguments of
    the model must have the same batch size as the examples, and the i-th
    example will be given to the model with the i-th index of each additional
    argument.
    ...
    with torch.autograd.set_grad_enabled(True):
        ...
        for start in trange(0, X.shape[0], batch_size, disable=not verbose):
            ...
            with torch.autocast(device_type=device, dtype=dtype):
                ...
                if len(y_.shape) > 1:
                    y_ = torch.mean(y_, dim=tuple(range(1, len(y_.shape))))

                y_like = torch.ones_like(y_)
                y_scaled = scaler.scale(y_)

                y_grad_scaled = torch.autograd.grad(y_scaled, X_, grad_outputs=y_like)[
                    0
                ]
...
def _attribution_score(grads):
    ...
    This function, which is meant to be used for Taylor approximated ISM,
    will take in the gradients and return the position-normalized differences.
    ...
    attr: torch.Tensor, shape=(-1, len(alphabet), length)
            The Taylor approximated ISM attributions for each input example.
    """
...
def taylor_ism(
    ...
    -------
    attr: torch.Tensor, shape=(-1, len(alphabet), length)
            The Taylor approximated ISM attributions for each of the input
            sequences.
